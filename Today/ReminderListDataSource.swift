...
//  Today
//
//  Created by Daniel Smith on 4/24/21.
//
...
class ReminderListDataSource: NSObject {
    ...
    init(reminderCompletedAction: @escaping ReminderCompletedAction, reminderDeletedAction: @escaping ReminderDeletedAction, remindersChangedAction: @escaping RemindersChangedAction) {
        ...
        requestAccess(completion: { (authorized) in
            if authorized {
    ...
    func update(_ reminder: Reminder, at row: Int, completion: (Bool) -> Void) {
        saveReminder(reminder, completion: { (id) in
            let success = id != nil
    ...
    func delete(at row: Int, completion: (Bool) -> Void) {
        ...
        removeReminder(with: reminder.id, completion: { (success) in
            if success {
                let index = self.index(for: row)
...
extension ReminderListDataSource {
    ...
    private func readAllReminders() {
        ...
        eventStore.fetchReminders(matching: predicate, completion: { (ekReminders) in
            ...
            self.reminders = ekReminders.compactMap {
                ...
                let reminder = Reminder(id: $0.calendarItemIdentifier, title: $0.title, dueDate: dueDate, notes: $0.notes, isComplete: $0.isCompleted)
                return reminder
            }
    ...
    private func readReminder(with id: String, completion: (EKReminder?) -> Void) {
        ...
        guard let calendarItem = eventStore.calendarItem(withIdentifier: id), let ekReminder = calendarItem as? EKReminder else {
            completion(nil)
            return
        }
